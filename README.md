...
A set of eigenfaces can be generated by performing a mathematical process called principal component analysis (PCA) on a large set of face images. Eigenfaces can be considered as a set of "standardized face ingredients". Any face is a combination of these standard faces. For example, one's face might be composed of the average face plus 15% of eigenface 1, 60% of eigenface 2, and −5% of eigenface 3. These percentages are coefficients or weights which are then used to recognize faces. When a new face is presented for classification, its own weights are found by projecting the image onto the collection of eigenfaces. This provides a set of weights describing that face. These weights are then compared with the weights of each image face in order to find the closest match. The Euclidean distance between two vectors with weights is used.
...
The weights of `i` test face are compared with the weights of each training face in order to find the closest match. The Euclidean distance between two vectors with weights is used.
